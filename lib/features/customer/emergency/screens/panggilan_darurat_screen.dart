...
class _PanggilanDaruratScreenState extends State<PanggilanDaruratScreen> {
  ...
  Future<void> _updateLocationData(double lat, double lon) async {
    ...
    try {
      final nearest = await _service.getNearestWorkshop(
        latitude: lat,
        longitude: lon,
      );
      ...
      try {
        ...
        try {
          ...
          final response = await http
              .get(url, headers: {'User-Agent': 'MotoCareApp'})
          ...
          if (response.statusCode == 200) {
            ...
            if (data != null && data['address'] != null) {
              ...
              if (addr['suburb'] != null || addr['village'] != null)
                parts.add(addr['suburb'] ?? addr['village']);
              if (addr['city'] != null ||
                  addr['town'] != null ||
                  addr['county'] != null)
                parts.add(addr['city'] ?? addr['town'] ?? addr['county']);
              if (parts.isNotEmpty) {
                address = parts.join(', ');
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () async {
                      final vehicle = await PilihKendaraanBottomSheet.show(
                        context,
                      );
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: _submittingType != null
                    ? null
                    : () => _submitEmergency('mechanic'),
                icon: _submittingType == 'mechanic'
                    ? const SizedBox(
                        width: 18,
                        height: 18,
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  disabledBackgroundColor: AppColors.dangerDark.withValues(
                    alpha: 0.5,
                  ),
                ),
                label: Text(
        ...
        child: _loadingNearest
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                          '${(_nearestWorkshop!['distance_meters'] ?? 0).toString()} m',
                          style: AppTheme.bodySmall,
                        ),
                      ],
                    ),
            ...
            : Center(
                child: Padding(
                  ...
                  child: Text(
                    'Tidak ada bengkel yang tersedia saat ini.',
                    style: AppTheme.bodySmall,
                  ),
                ),
              ),
  ...
  Future<void> _submitEmergency(String type) async {
    ...
    if (_selectedVehicle == null) {
      if (_merkController.text.trim().isEmpty ||
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Semua detail kendaraan wajib diisi jika tidak memilih dari daftar.',
            ),
    ...
    try {
      ...
      final resp = await _service.createEmergency(
        ...
        vehicleBrand: _selectedVehicle == null
            ? (_merkController.text.trim().isEmpty
                  ? null
                  : _merkController.text.trim())
            : null,
        vehicleType: _selectedVehicle == null
            ? (_tipeController.text.trim().isEmpty
                  ? null
                  : _tipeController.text.trim())
            : null,
        vehicleModel: _selectedVehicle == null
            ? (_modelController.text.trim().isEmpty
                  ? null
                  : _modelController.text.trim())
            : null,
        plateNumber: _selectedVehicle == null
            ? (_platController.text.trim().isEmpty
                  ? null
                  : _platController.text.trim())
            : null,
        complaint: _keluhanController.text.trim().isEmpty
            ? null
      ...
      if (resp['success'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(resp['message'] ?? 'Permintaan emergency berhasil'),
          ),
        );
        Navigator.pushAndRemoveUntil(
          ...
          MaterialPageRoute(
            builder: (context) => MainWrapper(
              daruratType: type == 'towing' ? 'towing' : 'mekanik',
            ),
          ),
          (route) => false,
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(resp['message'] ?? 'Gagal membuat permintaan'),
          ),
        );
      }
    } catch (e) {
